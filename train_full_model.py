...
class Config:
    ...
    OUTPUT_DIR = "output/all_model_LoRA"
...
class MOSEIDataset(Dataset):
    ...
    def _prepare_data(self):
        ...
        for segment_id in self.aligned_features.keys():
            ...
            try:
                visual = self.aligned_features[segment_id]['visual'].astype(np.float32)
                acoustic = self.aligned_features[segment_id]['acoustic'].astype(np.float32)
...
class LingoAuraLLM(nn.Module):
    def __init__(self, config):
        ...
        self.llm = AutoModelForCausalLM.from_pretrained(
            config.LLM_NAME,
            torch_dtype=torch.float16, # <-- 明确指定使用 float16 半精度
            device_map="auto",
        ...
        self.visual_projector.to(llm_device, dtype=torch.float16)
        self.acoustic_projector.to(llm_device, dtype=torch.float16)
    ...
    def forward(self, input_ids, attention_mask, labels, visual_features, acoustic_features):
        ...
        projected_acoustic = self.acoustic_projector(acoustic_features)
...
def create_collate_fn(tokenizer):
    def collate_fn(batch):
        ...
        return {'input_ids': tokenized['input_ids'], 'attention_mask': tokenized['attention_mask'], 'labels': labels,
                'visual_features': pad_sequence([f['visual'] for f in batch], batch_first=True).to(torch.float16),
                'acoustic_features': pad_sequence([f['acoustic'] for f in batch], batch_first=True).to(torch.float16)}
    return collate_fn
...
def main():
    ...
    collate_fn = create_collate_fn(AutoTokenizer.from_pretrained(config.LLM_NAME, trust_remote_code=True))
    ...
    # 只保存可训练的部分以节省空间
    torch.save(model.visual_projector.state_dict(), os.path.join(final_path, "visual_projector.pt"))
    torch.save(model.acoustic_projector.state_dict(), os.path.join(final_path, "acoustic_projector.pt"))
    print(f"\n训练完成！可训练权重已保存至: {final_path}")
