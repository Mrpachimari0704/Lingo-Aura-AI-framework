...
class LingoAuraInferenceModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, visual_features, acoustic_features):
        # 1. 生成 fake 特征并处理（原有逻辑不变）
        dummy_index = torch.zeros(input_ids.shape[0], 1, dtype=torch.long, device=input_ids.device)
        fake_visual_feature = self.fake_visual_token(dummy_index)
        fake_acoustic_feature = self.fake_acoustic_token(dummy_index)

        projected_visual = self.visual_projector(fake_visual_feature.to(torch.bfloat16))
        projected_acoustic = self.acoustic_projector(fake_acoustic_feature.to(torch.bfloat16))
        ...
        inputs_embeds = torch.cat([
            text_embeds[:, :1, :],  # 文本首token
            visual_token_embeds,    # 视觉token
            acoustic_token_embeds,  # 声学token
            text_embeds[:, 1:, :]   # 文本剩余部分
        ], dim=1)
        
        # 3. 扩展注意力掩码（与训练时完全一致）
        extra_tokens_mask = torch.ones((attention_mask.shape[0], 2), device=attention_mask.device)
        final_attn_mask = torch.cat([
            attention_mask[:, :1],  # 文本首token掩码
            extra_tokens_mask,      # 视觉/声学token掩码
            attention_mask[:, 1:]   # 文本剩余部分掩码
        ], dim=1)

        # 4. 修正生成逻辑：删除 start_positions，通过输入长度控制生成起点
        outputs = self.base_model.generate(
            ...
            do_sample=False,
            temperature=0.7,  # 适当提高温度，避免生成空内容
            min_new_tokens=1
        )
        
        # 5. 手动截取生成内容（确保只保留新增部分）
        # 输入总长度 = 原始文本长度 + 2个模态token
        input_total_length = inputs_embeds.shape[1]
        generated_tokens = [out[input_total_length:] for out in outputs]  # 只取新增生成的部分
        return generated_tokens  # 返回截取后的token，避免包含输入部分
# --- 主评估函数 ---
def evaluate():
    print("="*60)
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Evaluating on Test Set"):
            ...
            responses = responses = tokenizer.batch_decode(outputs, skip_special_tokens=True)
