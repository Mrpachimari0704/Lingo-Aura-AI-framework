...
# --- 1. 配置 ---
class Config:
    MODEL_PATH = "output/ablation_text_only_addcognitive"
...
# --- 2. 推理专用数据处理 (纯文本版) ---
class EvaluationDataset(Dataset): # 建议改名为 EvaluationDataset 以示区分
    ...
    def _prepare_data(self):
        ...
        for segment_id in self.split_ids:
            try:
                ...
                # ✨✨✨【核心修改】: 返回 'prompt' 和 'ground_truth_score' ✨✨✨
                prepared_data.append({
                    'prompt': human_prompt + "### Assistant:", # 保持和旧版一致，包含 Assistant 触发词
...
def evaluate_text_only():
    ...
    test_dataset = EvaluationDataset(cognitive_df, md.cmu_mosei.standard_folds.standard_test_fold, prompt_template)
