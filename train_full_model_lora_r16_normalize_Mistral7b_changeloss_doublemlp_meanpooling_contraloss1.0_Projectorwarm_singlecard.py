# ==============================================================================
#  lingo_aura_standalone_single_no_accum.py (单卡标准版 - 无梯度累积)
# ==============================================================================
...
class Config:
    ...
    # [显存注意]：去掉了梯度累积，这里的 Batch Size 就是真实的更新量
    # 24G 显存通常可以设为 4 或 8 (取决于序列长度)
    # 如果 OOM (显存溢出)，请将此值改为 2 或 1
    BATCH_SIZE = 32             
...
class LingoAuraLLM(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels, visual_features, acoustic_features):
        ...
        total_loss = outputs.loss + 1.0 * contrast_loss
        return total_loss
...
# --- 辅助函数：创建优化器 (修改点：不再除以 Accumulation Steps) ---
def create_optimizer_and_scheduler(model, epochs_remaining, steps_per_epoch):
    ...
    # 【修改点】Total Steps 就是 Batch 总数，不用除了
...
def main():
    ...
    train_loader = DataLoader(train_dataset, batch_size=config.BATCH_SIZE, 
                              shuffle=True, collate_fn=collate_fn, num_workers=16, pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=config.BATCH_SIZE, 
                            shuffle=False, collate_fn=collate_fn, num_workers=16, pin_memory=True)
    ...
    for epoch in range(config.EPOCHS):
        ...
        for batch in progress_bar:
            ...
            # 4. 反向传播 (不需要除以 accumulation_steps)
            loss.backward()
