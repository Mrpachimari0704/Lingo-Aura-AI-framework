...
class Config:
    ...
    OUTPUT_DIR = "output/all_model_LoRA_attention_right_label_r16_visionpre_novision"
...
class LingoAuraLLM(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels, visual_features, acoustic_features):
        ...
        inputs_embeds = torch.cat([acoustic_token_embeds, text_embeds], dim=1)
        extra_tokens_mask = torch.ones((attention_mask.shape[0], 1), device=attention_mask.device)
        final_attn_mask = torch.cat([extra_tokens_mask, attention_mask], dim=1)
        extra_labels = torch.full((labels.shape[0], 1), -100, device=labels.device)
