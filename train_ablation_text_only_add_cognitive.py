...
class Config:
    ...
    PROMPT_TEMPLATE_PATH = "./prompts/cognitive_informed_prompt.txt" # 使用简单模板
    OUTPUT_DIR = "output/ablation_text_only_addcognitive" # ✨✨✨ 独立的输出目录 ✨✨✨
    ...
    def _prepare_data(self):
        ...
        for segment_id in self.split_ids:
            try:
                ...
                # --- ✨✨✨【核心修改】: 使用包含认知信息的新 prompt ✨✨✨ ---
                human_prompt = self.prompt_template.split("### Assistant:")[0].format(
                    # 从解析后的 cognitive_label 字典中获取值
                    # .get(..., ["N/A"])[0] 是一种安全写法，确保即使键不存在或值为空列表，也能得到一个默认值
                    information_stance=cognitive_label.get("Information Stance", ["N/A"])[0],
                    reasoning_mode=cognitive_label.get("Reasoning Mode", ["N/A"])[0],
                    transcription=text
                ).strip() # strip() 移除前后可能的多余空白

                assistant_response = self.prompt_template.split("### Assistant:")[1].format(emotion_score=emotion_score).strip()
                
                prepared_data.append({
                    'full_text': human_prompt + " " + assistant_response, # 中间加个空格更规范
                    'prompt_len': len(human_prompt)
                })
            except Exception as e:
                # print(f"Skipping segment {segment_id} due to error: {e}") # 可选，用于调试
                continue
