...
class Config:
    ...
    OUTPUT_DIR = "output/all_model_LoRA_doublemlp_meanpool_contrastive_ddp_contrastloss1.0_Projectorwarm" # 更新输出目录名
...
class LingoAuraLLM(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels, visual_features, acoustic_features):
        ...
        # visual_text_sim = torch.clamp(visual_text_sim, max=80.0) 
        # acoustic_text_sim = torch.clamp(acoustic_text_sim, max=80.0)
        ...
        # 总损失 = 生成损失 + 0.1 * 对比损失
        total_loss = outputs.loss + 1 * contrast_loss
        return total_loss
...
def main():
    ...
    train_loader = DataLoader(train_dataset, batch_size=config.BATCH_SIZE, 
                              sampler=train_sampler, collate_fn=collate_fn, num_workers=8, pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=config.BATCH_SIZE, 
                            sampler=val_sampler, collate_fn=collate_fn, num_workers=8, pin_memory=True)
